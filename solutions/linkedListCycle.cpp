...
bool containsCycle(const LinkedListNode* firstNode)
{
    ...
    //Make two runners where one starts at beginning and the other starts at the next node
    const LinkedListNode* slow = firstNode;
    const LinkedListNode* fast = firstNode->next_;
    //loop until fast or fast next is nullptr
    while(fast && fast->next_){ 
        if(slow == fast) return true; //if slow equals fast that means there is a cycle
        slow = slow->next_; //increment slow
        fast = fast->next_->next_; //increment fast so it will be 2x as fast
    }
    return false;
}
