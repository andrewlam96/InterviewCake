...
LinkedListNode* reverse(LinkedListNode* headOfList)
{
    ...
    //if head is nullptr then return nullptr
    if(!headOfList) return nullptr;

    //need nodes for head and make a node for previous node
    LinkedListNode* curr = headOfList;
    LinkedListNode* prev = nullptr;
    
    //go through list
    while(curr){
        //have a temp node for the next node
        LinkedListNode* temp = curr->next_;

        //set the next node as the previous
        curr->next_ = prev;

        //update previous to current and current to current next
        prev = curr;
        curr = temp;
    }
